...
GlobalValue g_checksumEnabled  = GlobalValue ("ChecksumEnabled",
                                              ...
                                              BooleanValue (false),
                                              MakeBooleanChecker ());
std::vector<Ptr<NetDevice> > Node::GetDevices(){
        return m_devices;
}
...
double Node::calcRx(){
    int count = 0;
   // Ptr<MobilityModel> mob = GetObject<MobilityModel> ();
    for (std::vector<Ptr<NetDevice> >::iterator it = m_devices.begin(); it != m_devices.end(); ++it){
        ...
        for (std::vector<Ptr<YansWifiPhy> >::const_iterator i = channel->m_phyList.begin (); i != channel->m_phyList.end (); i++)
            {
                ...
                //Ptr<MobilityModel> receiverMobility = yansWifiPhy->GetMobility ()->GetObject<MobilityModel> ();
//                Ptr<MobilityModel> receiverMobility = yansWifiPhy->m_mobility->GetObject<MobilityModel> ();
                Ptr<Object> receiverMobility = yansWifiPhy->m_mobility;
                cout << "receiverMobility: " << receiverMobility <<endl;
                //channel ->customCalcRxPower(this, receiverMobility);
...
Ptr<Node> Node::getNodeByAddr(Ipv4Address addr){
