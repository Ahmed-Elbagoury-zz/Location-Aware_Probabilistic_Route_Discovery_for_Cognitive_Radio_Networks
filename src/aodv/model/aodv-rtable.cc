...
RoutingTable::RoutingTable (Time t, Ptr<Node> theCurNode) : 
  m_badLinkLifetime (t)
{
        curNode = theCurNode;
}
...
{
  NS_LOG_FUNCTION (this);
//cout << curNode->GetId () << rt.GetSource () << rt.GetDestination () <<  rt.GetNextHop () << rt.GetHop () << endl;
//cout << rt.GetSource () <<  "\t" << rt.GetDestination () << "\t" << rt.GetNextHop () <<  "\t" <<rt.GetHop () << endl;
  Purge ();
