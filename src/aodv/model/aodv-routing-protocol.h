...
#define PT 17.0206
#define PINT -96.0
#define SEGMA_N_SQUARE -99.0
//TODO to be converted from DBm
#define Free_space_Path_Loss_constant   1.0
#define BETA    0.5
//TODO to be tuned
#define PI_VAL 3.141592653589793238
using namespace std;
...
//Beamforming methods
  double calcRx();
  void processRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src);
  double clacCoefficient ();
  std::vector<Ipv4Address> getNeighbors ();
  double calcMetric();
  double getNpThreshold(int groupSize);
  void getPowerSet(int maxGroupSize, int ind, std::vector<std::vector<Ipv4Address> >& powerSet, std::vector<Ipv4Address>& neighbors, std::vector<int> & marked);
  void sendPU();
  void RecvPU(Ipv4Address sender);
...
private:
  ...
  //\{
  std::set<pair<Ipv4Address, uint16_t > > flows;
...
private:
  ...
  */
  ...
  Ptr<Node> getNodeByAddr(Ipv4Address addr);
