...
//  AODV_PU = 5           //Packet sent from a PU
};
...
public:
  ...
  void setNeighbors(std::vector<Ipv4Address> theNeighbors) {neighbors = theNeighbors;};
  void setFlows(std::set<std::pair<Ipv4Address, uint16_t> > newFlows) {flows = newFlows;};
  Ipv4Address getOrigin(){return m_origin;}
private:
  ...
  std::set<std::pair<Ipv4Address, uint16_t> > flows;     
};
