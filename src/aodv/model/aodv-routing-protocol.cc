...
void RoutingProtocol::RecvPU(Ipv4Address sender){
        cout << "Node " << getNodeByAddr(sender)->GetId() << " received PU packet"<< endl;
}
double RoutingProtocol::calcRx(){
        std::vector<Neighbors::Neighbor> neighbors = m_nb.m_nb;
        map<Ipv4Address, Neighbors::Neighbor> neighborsAddr;
        //Hash the neighbors
        for (std::vector<Neighbors::Neighbor>::const_iterator i = neighbors.begin ();i != neighbors.end (); ++i){
                neighborsAddr.insert ( std::make_pair((*i).m_neighborAddress, (*i)) );
                //cout << "In node: " << curNode->GetId() << ", neighbor is " <<  ((*i).m_neighborAddress) << endl;
        }

        std::vector<Ptr<NetDevice> > devices = curNode ->GetDevices();
        for (std::vector<Ptr<NetDevice> >::iterator it = devices.begin(); it != devices.end(); ++it){
        Ptr<Channel> channel = (*it)->GetChannel();
        if(channel == 0)
                continue;
        std::vector<Ptr<YansWifiPhy> > i = channel->m_phyList;
        for (std::vector<Ptr<YansWifiPhy> >::const_iterator i = channel->m_phyList.begin (); i != channel->m_phyList.end (); i++){
                Ptr<YansWifiPhy> yansWifiPhy = (*i);
                Ptr<MobilityModel> receiverMobility = yansWifiPhy->m_mobility->GetObject<MobilityModel> ();
                Ptr<Node> recvNode = receiverMobility->GetObject<Node> ();
                if(curNode->GetId() == recvNode->GetId() )
                        continue;
                //Get node address and check if it is in the neighbor list or not
                Ptr<Ipv4> ipv4 = recvNode->GetObject<Ipv4>();
                Ipv4InterfaceAddress iaddr = ipv4->GetAddress (1,0);
                Ipv4Address neighborAddr = iaddr.GetLocal (); 
                std::map<Ipv4Address, Neighbors::Neighbor>::iterator curNeighbor = neighborsAddr.find(neighborAddr);
                if(curNeighbor == neighborsAddr.end())
                        continue;
                Ptr<PropagationLossModel> propagationLossModel = channel -> m_loss;
                Ptr<MobilityModel> senderMobility = curNode->GetObject<MobilityModel> ();
                double val = propagationLossModel-> customCalcRxPower(senderMobility, receiverMobility);
                //Update the neighbor info to include the channel coeficient
                curNeighbor->second.coeficient = val;
                //cout << "In node: " << curNode->GetId() << ", to node: " << neighborAddr << ", val is " << val << endl;
                }
        }
        //cout << "###############################" << endl;
        double num = 0;
...
Ptr<Node> RoutingProtocol::getNodeByAddr(Ipv4Address addr){
        std::vector<Ptr<Node> > nodeList = curNode->getNodeList();
                
        for(std::vector<Ptr<Node> >::const_iterator i = nodeList.begin(); i != nodeList.end(); i++){
                if(addr == (*i)-> addr)
                   return (*i);        
        }
        return NULL;
...
void RoutingProtocol::processRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src){
          NS_LOG_FUNCTION (this);
  RreqHeader rreqHeader;
  p->RemoveHeader (rreqHeader);
  //Get previous hop location
  Vector prevPos = getNodeByAddr(src)->GetObject<MobilityModel> ()->GetPosition();
  Vector curPos = getNodeByAddr(receiver)->GetObject<MobilityModel> ()->GetPosition();
  Vector dstPos = getNodeByAddr(rreqHeader.GetDst ())->GetObject<MobilityModel> ()->GetPosition();



//  cout << "AODV node " << curNode->GetId() << " received a AODV request from " << src << " to " << receiver << " to dest " << reqDst <<endl;
//  if(srcMob != NULL && dstMob != NULL) cout << "AODV node " << curNode->GetId() << " received a AODV request from " << srcMob->GetPosition() << " to " << dstMob->GetPosition() << endl; 
//  else cout << "NULL!" << endl;
      
  //calculate the distances
  //1-from current node to final destination
  double destFromCur = (dstPos.x-curPos.x) *(dstPos.x-curPos.x) + (dstPos.y-curPos.y)* (dstPos.y-curPos.y)  + (dstPos.z-curPos.z)* (dstPos.z-curPos.z);
  double destFromPrev = (dstPos.x-prevPos.x) *(dstPos.x-prevPos.x) + (dstPos.y-prevPos.y)* (dstPos.y-prevPos.y)  + (dstPos.z-prevPos.z)* (dstPos.z-prevPos.z);
  if(destFromCur > destFromPrev){
        cout << "Dropping RREQ" << endl;
        return;
  }
  else cout << "Passed test" << endl;   

  //Select potential next hops
  std::vector<Ipv4Address> potentialNeighbors;
  double destFromNeighbor = 0;
  std::vector<Ipv4Address> neighbors = getNeighbors();
  Vector neighborPos = getNodeByAddr(src)->GetObject<MobilityModel> ()->GetPosition();
  for (std::vector<Ipv4Address>::const_iterator i = neighbors.begin ();i != neighbors.end (); ++i){
     neighborPos = getNodeByAddr(*i)->GetObject<MobilityModel> ()->GetPosition();
     destFromNeighbor = (dstPos.x-neighborPos.x) *(dstPos.x-neighborPos.x) + 
            (dstPos.y-neighborPos.y)* (dstPos.y-neighborPos.y)  + (dstPos.z-neighborPos.z)* (dstPos.z-neighborPos.z);
     if(destFromCur > destFromNeighbor)
        potentialNeighbors.push_back(*i);
  }
  //Max Group size determination
  int minGroupSize =  /**Np +**/1; //TODO Np
  int maxGroupSize = minGroupSize + 1;
  double bestMetric = 0;
  double fn = flows.size() / neighbors.size();
  double Nmin = fn * minGroupSize;
  cout << Nmin << ", "<<maxGroupSize << ", " << bestMetric<<endl;
  double factor = (fn * maxGroupSize - Nmin) / Nmin;
  while(factor < getNpThreshold (maxGroupSize)){
    maxGroupSize++;
  }
  std::vector<std::vector<Ipv4Address> > powerSet;
  std::vector<int> marked (neighbors.size(), 0);
  getPowerSet(maxGroupSize, 0, powerSet, neighbors, marked);
}

void RoutingProtocol::getPowerSet(int maxGroupSize, int ind, std::vector<std::vector<Ipv4Address> >& powerSet, std::vector<Ipv4Address>& neighbors, std::vector<int> & marked){
    if(ind > maxGroupSize){
        std::vector<Ipv4Address> cur;
        for(unsigned int i = 0; i < neighbors.size(); i++){
            if(marked[i])            
                cur.push_back(neighbors[i]);
        }
        return;
    }
    //Skip the current neighbor
    getPowerSet(maxGroupSize, ind+1, powerSet, neighbors, marked);
    //Add the current neighbor
    marked[ind] = 1;
    getPowerSet(maxGroupSize, ind+1, powerSet, neighbors, marked);
    marked[ind] = 0;   
}

double RoutingProtocol::calcMetric(){
  //TODO
  double area = 1.0; //TODO     
  double df = flows.size()/area;
  double wi = clacCoefficient();
  double dr2 = (Free_space_Path_Loss_constant*PT*wi) / PINT;
  //claculate ||w_i||^2
  double nf = df * PI_VAL * dr2;
  cout << nf << endl;
  return nf;
}

double RoutingProtocol::getNpThreshold(int groupSize){
    //TODO
    return 1;
}


std::vector<Ipv4Address> RoutingProtocol::getNeighbors(){
    //Create a vector of the neighbors
      std::vector<Ipv4Address> neighbors;
      for (std::vector<Neighbors::Neighbor>::const_iterator i = m_nb.m_nb.begin ();i != m_nb.m_nb.end (); ++i){
        neighbors.push_back((*i).m_neighborAddress);
      }
      return neighbors;
}

double RoutingProtocol::clacCoefficient (){
        //TODO
        return 1.0;
}

void
RoutingProtocol::sendPU ()
{
  NS_LOG_FUNCTION (this);
...
RoutingProtocol::RoutingProtocol () :
  ...
  EnableHello (false),
  m_routingTable (DeletePeriod, curNode),
...
{
//Ptr<MobilityModel> senderMobility = curNode->GetObject<MobilityModel> ();
  //Ptr<Node> prevHop = getNodeByAddr();

//Incoming packet and we need to form a route
//  cout << "Before calling calcRx...." << endl;
//  calcRx();
...
**/
  ...
  if (m_routingTable.LookupValidRoute (dst, rt))
    {
      //cout << curNode->GetId() << "\t" << rt.GetSource ()  << "\t" << rt.GetDestination() << "\t" << rt.GetNextHop() << endl;
      route = rt.GetRoute ();
      NS_ASSERT (route != 0);
...
Ptr<Ipv4Route> tmpRoute = LoopbackRoute (header, oif);
  return tmpRoute;
}
...
{
//cout << "Size = " << curNode->getNodeList().size() << endl;
Ptr<Node> s = getNodeByAddr(header.GetSource ());
Ptr<Node> d = getNodeByAddr(header.GetDestination () );
if(d != NULL && s != NULL){
        if( (d->isPU() || s->isPU() ) && (curNode -> GetId () != s->GetId() && curNode -> GetId () != d->GetId()))  cout << "S " << s->GetId() << ", D " << d->GetId() << ", current " << curNode->GetId() << endl;
}
/**
if(curNode -> isPU()){
        //cout << "PU " << curNode->GetId() <<  ", received packet from " << s->GetId() <<endl;
        return NULL;
  }
        
  if(s -> isPU()){
        //cout << "Node " << curNode->GetId() <<  ", received PU packet from " << s -> GetId() <<endl;
        return NULL;
  }
**/
/**
if( s != NULL && d != NULL)
        cout << curNode->GetId() << "\t" << s->GetId() << "\t" << d->GetId() << endl;
**/

//cout << curNode->GetId() << "\t" << s == NULL ? "NULL" : s->GetId() << "\t" << d == NULL ? "NULL" : d->GetId() << endl;
...
//        cout << "In node " << curNode->GetId() << ", dest : " << header.GetDestination ()<< ", src: " <<header.GetSource ()<< endl;
        //cout << "Recv. In node " << curNode -> GetId() << ". " << header.getDescription () <<endl;
        //cout << "---------------" << endl;
        pair<Ipv4Address, uint16_t> curFlow (header.GetSource(), header.GetIdentification());
        flows.insert(curFlow);
   }
...
//        cout << "My packt \t"<<curNode -> GetId () << "\t" << origin << "\t" << dst << endl;
    return true;
...
/**
  if(curNode -> isPU()){
        //cout << "PU " << curNode->GetId() <<  ", received PU packet from " << getNodeByAddr(sender)->GetId() <<endl;
        return;
  }
        
  if(getNodeByAddr(sender) -> isPU()){
        //cout << "Node " << curNode->GetId() <<  ", received PU packet"<<endl;
        return;
  }
**/
  ...
  switch (tHeader.Get ())
    ...
    case AODVTYPE_RREQ:
      {
        //cout << "AODV node " << curNode->GetId() << " received a AODV request from " << sender << " to " << receiver << endl;
        RecvRequest (packet, receiver, sender);
        break;
      }
    case AODVTYPE_RREP:
      {
//cout << "AODV node " << curNode->GetId() << " received a AODV packet from " << sender << " to " << receiver << endl;
...
//cout << "AODV node " << curNode->GetId() << " received RREQ " << src << " to " << receiver << endl;
...
//  else cout << "NULL!" << endl;
  ...
  if(destFromCur > destFromPrev) cout << "Dropping RREQ" << endl;
  else cout << "Passed test" << endl;
**/
        /**-------------------------------------------------**/
...
//cout << "Node " <<getNodeByAddr(helloHeader.getOrigin()) -> GetId() << " is " << getNodeByAddr(helloHeader.getOrigin()) -> isPU() << endl;
Ptr<Node> source = getNodeByAddr(rrepHeader.getOrigin());

/**
if(source != NULL){
        if(source->isPU()){
                cout << "Node " << curNode -> GetId() << " Recv RREP from PU " << source -> GetId() << endl;
                return;
        }
}
**/
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      //Create a vector of the neighbors
      std::vector<Ipv4Address> neighbors = getNeighbors();     
      helloHeader.setNeighbors(neighbors);
      //set the flows in the hello packets
      std::set<std::pair<Ipv4Address, uint16_t> > newFlows;
      newFlows.insert(flows.begin(), flows.end());
      helloHeader.setFlows(newFlows);  
      //Clear the contents of the old flows map
      flows.clear();  
