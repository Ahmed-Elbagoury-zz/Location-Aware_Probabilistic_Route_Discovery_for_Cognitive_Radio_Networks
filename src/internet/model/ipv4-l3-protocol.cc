...
/**
  ...
  Ptr<Node> origin = m_node ->getNodeByAddr(h.GetDestination() );
  
  Ptr<Node> s = getNodeByAddr(header.GetSource ());
  Ptr<Node> d = getNodeByAddr(header.GetDestination () );
  if(d != NULL && d != NULL){
        if(d->isPU() || s->isPU())  cout << "S " << s->GetId() << ", D " << d->GetId() << ", current " << curNode->GetId() << endl;
  }
...
**/
  ...
  packet->RemoveHeader (ipHeader);
//cout << m_node->GetId () << "\t" << ipHeader.GetSource() << "\t" << ipHeader.GetDestination () << "\t" <<p->GetUid ()<<endl; //Cur src dst pkt->uid
